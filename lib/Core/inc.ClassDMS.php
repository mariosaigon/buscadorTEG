...
$searchAttributes = array();
			...
			//EN MI LOCAL EL ATRIBUTO FECHA DE RESOLUCION ES ID 1 EN LA BD
			//echo "fecha inicial: ".$fecha_final_formateada;
			$searchAttributes[] = "EXISTS (SELECT NULL FROM `tblDocumentAttributes` WHERE `tblDocumentAttributes`.`attrdef`= 1 AND (`tblDocumentAttributes`.`value` ) between '".$fecha_inicial_formateada."' and '".$fecha_final_formateada."' AND `tblDocumentAttributes`.`document` = `tblDocuments`.`id`)";
