...
$vengoAvanzada=FALSE;
if (isset($_GET["vengoDeBusquedaAvanzada"])) 
{
   $vengoAvanzada=TRUE;
}

$limiteResultados=10;
if (isset($_GET["limiteResultados"])) 
{
   $limiteResultados=$_GET["limiteResultados"];
}
...
/*
	...
	if(isset($_GET["fechaInicio"])) 
	{
		...
		{
		$quiero_buscar_rango=TRUE;
		//echo "definido el atributo fechaINicio=> ".$_GET["fechaInicio"];
		$tmp = explode("-", $_GET["fechaInicio"]);
		//print_r($tmp);
		$fecha_inicial = array('year'=>(int)$tmp[2], 'month'=>(int)$tmp[1], 'day'=>(int)$tmp[0], 'hour'=>0, 'minute'=>0, 'second'=>0);
		}
	...
	if(isset($_GET["fechaFin"])) 
	{
		...
		$fecha_final = array('year'=>(int)$tmp[2], 'month'=>(int)$tmp[1], 'day'=>(int)$tmp[0], 'hour'=>23, 'minute'=>59, 'second'=>59);
		}
...
if($settings->_showSingleSearchHit && count($entries) == 1) {
	...
	if($view) {
		...
		$view->setParam('limiteResultados', $limiteResultados);
		$view->setParam('vengoAvanzada', $vengoAvanzada);
		$view->setParam('quieroRango', $quiero_buscar_rango);
		if($quiero_buscar_rango==TRUE)
		{

			$view->setParam('fechaInicio', $_GET["fechaInicio"]);
			$view->setParam('fechaFin', $_GET["fechaFin"]);
		}
		$view($_GET);
		exit;
	}
