...
for ($file_num=0;$file_num<count($_FILES["userfile"]["tmp_name"]);$file_num++){
	...
	if (count($_FILES["userfile"]["tmp_name"])==1) {
		$name_correlative = (string)$_POST["name-correlative"];
		$name_origin = (string)$_POST["name-origin"];
		$name_year = (string)$_POST["name-year"];
 		
 		$name = trim($name_correlative."-".$name_origin."-".$name_year);
	} else {
	...
	//Que el comentario del fichero sea un extracto del problema jurídico
	// $atributoProblema=$dms->getAttributeDefinitionByName($stringProblema);
	// $idAtributo=$atributoProblema->getID();
