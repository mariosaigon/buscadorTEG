...
if ($user->isGuest()) {
	UI::exitError(getMLText("nonconfo_delete_owner"),getMLText("access_denied"));
}

if (!isset($_REQUEST["id"])) {
	UI::exitError(getMLText("nonconfo_delete_owner"),getMLText("error_occured"));
}
...
switch ($command) {
	case 'deleteowner':
		if(!checkFormKey('removeowner', 'GET')) {
