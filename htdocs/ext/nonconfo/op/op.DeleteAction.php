...
$res = delNonConfoAction($_REQUEST['actionId']);

if (false == $res || empty($res)) {
	UI::exitError(getMLText("nonconfo_delete_action"),getMLText("error_occured"));
}

$action = getNonConfoActionById($_POST['actionId']);

$session->setSplashMsg(array('type'=>'success', 'msg'=>getMLText('nonconfo_action_deleted')));
